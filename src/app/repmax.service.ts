...
export class RepmaxService {
  ...
  private fetchDayMaxes() {
    const dayMaxUrl = 'https://strengthpractice-7e443.firebaseio.com/daymaxes.json';
  ...
  private fetchRecordMaxes() {
    const recordMaxUrl = 'https://strengthpractice-7e443.firebaseio.com/recordmaxes.json';
  ...
  patchDayMaxes(workout: Workout) {
    workout.exercises.forEach(exercise => {
      ...
      if (exercise.sets.length < 1) { return; } else if (exercise.setType === 'clusters' || exercise.setType === 'mtor') {
        entry = {
          date: workout.date,
